...
create_anykernel_zip() {
    ...
    # Create zip
    ZIP_NAME="$KDIR/kernel_build/Floppy_${FK_VER}-${KERNEL_VARIANT}-${CODENAME}-${DATE}.zip"
...
create_odin_tar() {
    ...
    TAR_NAME="$KDIR/kernel_build/Floppy_${FK_VER}-${KERNEL_VARIANT}${variant_suffix}-${CODENAME}-${DATE}.tar"
