...
static struct isp_cooling_device *isp_dev = NULL;
...
static void exynos_isp_cdev_register(struct isp_cooling_device *isp_cdev)
{
	int id, ret, size, i;
	unsigned long fps;
	...
	exynos_acpm_tmu_set_isp_cdev(id, ISP_CDEV_ID, 0, isp_cdev->cool_dev->id);
	exynos_acpm_tmu_set_isp_cdev(id, ISP_CDEV_TZ_ID, 0, isp_cdev->tz->id);

	ret = get_property(0, 0, &num_level, GET_MAXL);
	if (ret)
		return;

	for (i = 0; i < num_level; i++) {
		fps = isp_fps_table[i].fps;
		exynos_acpm_tmu_set_isp_cdev(id, ISP_CDEV_FPS, i, fps);
	}

	size = isp_cdev->tz->num_trips;
	for (i = 0; i < size; i++) {
		instance = get_thermal_instance(isp_cdev->tz,
				isp_cdev->cool_dev, i);
		if (instance)
				 ...
				 unsigned long state)
{
	...
	if (isp_device->isp_state == state)
		return 0;

	dbg_snapshot_thermal(NULL, state, cdev->type, isp_cooling_get_fps(0, state));

	isp_device->isp_state = (unsigned int)state;

	blocking_notifier_call_chain(&isp_notifier, ISP_THROTTLING, &state);
...
int exynos_tmu_isp_add_notifier(struct notifier_block *n)
{
	...
	ret = blocking_notifier_chain_register(&isp_notifier, n);
	if (ret)
		return ret;
	...
	mutex_lock(&isp_dev->cool_dev->lock);
	state = isp_dev->isp_state;
	blocking_notifier_call_chain(&isp_notifier, ISP_THROTTLING, &state);
	mutex_unlock(&isp_dev->cool_dev->lock);

	return ret;
}
EXPORT_SYMBOL_GPL(exynos_tmu_isp_add_notifier);

int exynos_tmu_isp_remove_notifier(struct notifier_block *n)
{
	return blocking_notifier_chain_unregister(&isp_notifier, n);
}
EXPORT_SYMBOL_GPL(exynos_tmu_isp_remove_notifier);
			   ...
			   const struct cpumask *clip_isp)
{
	...
	isp_dev->tz = parse_ect_cooling_level(cool_dev, "ISP");

	isp_dev->cool_dev = cool_dev;
	isp_dev->isp_state = 0;

#if IS_ENABLED(CONFIG_EXYNOS_ESCA_THERMAL)
	exynos_isp_cdev_register(isp_dev);
#endif
	mutex_lock(&cooling_isp_lock);
