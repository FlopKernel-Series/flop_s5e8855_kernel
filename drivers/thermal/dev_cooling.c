...
struct dev_cooling_device {
	...
	u32					max_state;
	struct dvfs_rate_volt			*freq_table;
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCA_THERMAL)
static void exynos_dev_cdev_register(struct dev_cooling_device *dev)
{
	int id, i, size;
	...
	exynos_acpm_tmu_set_device_cdev(id, DEV_CDEV_ID, 0, dev->cool_dev->id);
	exynos_acpm_tmu_set_device_cdev(id, DEV_CDEV_TZ_ID, 0, dev->tz->id);
	exynos_acpm_tmu_set_device_cdev(id, DEV_CDEV_MAX, 0, dev->max_state);

	for (i = 0; i < dev->max_state; i++) {
		exynos_acpm_tmu_set_device_cdev(id, DEV_CDEV_FREQ,
				i, dev->freq_table[i].rate);
	}

	for (i = 0; i < 4; i++)
		name_data |= dev->domain_name[i] << (i * 8);
	exynos_acpm_tmu_set_device_cdev(id, DEV_CDEV_NAME, 0, name_data);

	size = dev->tz->num_trips;
	for (i = 0; i < size; i++) {
		instance = get_thermal_instance(dev->tz, dev->cool_dev, i);
				 ...
				 unsigned long state)
{
	...
	dev->dev_val = dev->freq_table[state].rate;
	dbg_snapshot_thermal(NULL, state, cdev->type, dev->dev_val);
#if IS_ENABLED(CONFIG_EXYNOS_ESCA_THERMAL)
	exynos_pm_qos_update_request_nosync(&dev->thermal_pm_qos_max, dev->dev_val);
#else
				 ...
				 unsigned long value)
{
	...
	for (i = 0; i < dev->max_state; i++) {
		if (dev->freq_table[i].rate <= value) {
			level = i;
			break;
		}
	}
...
parse_ect_cooling_level(struct thermal_cooling_device *cdev, char *tz_name)
{
	struct thermal_instance *instance;
	struct thermal_zone_device *tz;
	bool foundtz = false;
...
__dev_cooling_register(struct device_node *np, struct exynos_devfreq_data *data)
{
	...
	if (get_invalid_sample()) {
		pr_err("Cannot init dev_cooling (thermal invalid sample)");
	...
	dev->tz = parse_ect_cooling_level(cool_dev, cooling_name);

	dev->cool_dev = cool_dev;

#if IS_ENABLED(CONFIG_EXYNOS_ESCA_THERMAL)
	if (of_property_read_string(np, "devfreq_domain_name", &name)) {
		pr_err("%s: could not find devfreq_domain_name\n", __func__);
		kfree(dev);
		return ERR_PTR(-EINVAL);
	}
	strncpy(dev->domain_name, name, DOMAIN_NAME_LEN);
	exynos_dev_cdev_register(dev);
#endif
	return cool_dev;
}
