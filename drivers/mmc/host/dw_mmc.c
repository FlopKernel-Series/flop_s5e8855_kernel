...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_OFF:
		...
		if (mmc->unused && mmc->rescan_entered)	
...
static void dw_mci_sd_power_off(struct dw_mci *host)
{
...
static void dw_mci_work_routine_card(struct work_struct *work)
{
	...
	/* Turn the power off when SD card removed */
	if (!present) {
		dw_mci_sd_power_off(host);
	}
...
#if defined(CONFIG_SOC_S5E8855)
	host->cmu_addr = ioremap(S5E8855_CMU_TOP_BASE, 0x200);
#endif
