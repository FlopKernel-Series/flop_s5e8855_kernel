...
static void simulate_BAD_STACK_RE(char **argv, int argc);
...
enum {
	...
	FORCE_BAD_STACK_RE,		/* BAD STACK RECURSION */
	NR_FORCE_ERROR,
};
...
struct force_error force_error_vector = {
	.item = {
		...
		{"badstack-re",		&simulate_BAD_STACK_RE},
	}
};

static struct platform_device *sec_debug_test_pdev;
...
enum {
	TEST_PM_DUMMY_WAIT = 0,
	TEST_PM_BUSYLOOP,
	TEST_PM_DPM_WAIT,
};

static void do_test_dpmwait_func(void)
{
	if (!sec_debug_test_pdev) {
		pr_err("%s: no sec_debug_test_pdev\n", __func__);
		return;
	}

	pr_crit("%s: set async\n", __func__);
	device_enable_async_suspend(&sec_debug_test_pdev->dev);

	pr_crit("%s: reinit completion\n", __func__);
	reinit_completion(&sec_debug_test_pdev->dev.power.completion);

	pr_crit("%s: do dpm_wait\n", __func__);
	device_pm_wait_for_dev(&sec_debug_test_pdev->dev, &sec_debug_test_pdev->dev);
	pr_crit("%s: dpm_wait done\n", __func__);
}

static void do_test_lockup_func(int type)
{
	switch(type) {
	case TEST_PM_BUSYLOOP:
		pr_crit("%s: dp test busyloop\n", __func__);
		cpu_park_loop();
		break;
	case TEST_PM_DPM_WAIT:
		pr_crit("%s: dp test dpm wait\n", __func__);
		do_test_dpmwait_func();
		break;
	case TEST_PM_DUMMY_WAIT:
	default:
		pr_crit("%s: dp test dummy wait\n", __func__);
		dummy_wait_for_completion();
		break;
	}
}
...
static int shutdown_test_method;
...
static void do_shutdown_test(void)
{
	...
	do_test_lockup_func(shutdown_test_method);
}

static void simulate_SHUTDOWN_LOCKUP(char **argv, int argc)
{
	int method = TEST_PM_DUMMY_WAIT;
	bool testoff = false;

	if (argc > 0) {
		if (!strncmp(argv[0], "busyloop", 8))
			method = TEST_PM_BUSYLOOP;

		else if (!strncmp(argv[0], "dpmwait", 7))
			method = TEST_PM_DPM_WAIT;
	...
	if (!testoff) {
		...
		shutdown_test_method = method;
	} else {
		is_shutdown_test = false;
	}

	pr_info("%s: test %sabled (type:%d)\n", __func__,
			is_shutdown_test ? "en" : "dis",
			shutdown_test_method);
}

static bool is_suspend_test;
static bool is_resume_test;
static int suspend_resume_method;
...
static void do_suspend_test(void)
{
	...
	pr_info("%s: lockup test\n", __func__);
	do_test_lockup_func(suspend_resume_method);
}
...
static void simulate_SUSPEND_LOCKUP(char **argv, int argc)
{
	...
	if (argc > 0) {
		if (!strncmp(argv[0], "resume", 6))
			is_resume = true;

		else if (!strncmp(argv[0], "busyloop", 8))
			method = TEST_PM_BUSYLOOP;
	...
	if (testoff) {
		...
		suspend_resume_method = TEST_PM_DUMMY_WAIT;
	} else if (method > 0) {
		suspend_resume_method = method;
		if (!is_suspend_test && !is_resume_test)
			is_suspend_test = true;
	} else if (is_resume) {
		is_suspend_test = false;
		is_resume_test = true;
	} else {
		/* default suspend test */
		is_suspend_test = true;
		is_resume_test = false;
	}

	pr_info("%s: suspend test %sabled (type:%d)\n", __func__,
			is_suspend_test ? "en" : "dis",
			suspend_resume_method);
	pr_info("%s: resume test %sabled (type:%d)\n", __func__,
...
static void simulate_FLUSH_WQ(char **argv, int argc)
{
	...
	wq = alloc_workqueue("sec_test_flush_wq", 0, 10);
...
static void __maybe_unused secdbg_test_do_sp_pc_abort(void *data,
		unsigned long addr, unsigned long esr, struct pt_regs *regs)
{
	pr_crit("%s: try infinite exception(esr: 0x%lx)\n", __func__, esr);
...
static void __maybe_unused secdbg_test_do_handle_bad_stack(void *data, struct pt_regs *regs,
		unsigned long esr, unsigned long far)
{
	pr_crit("%s: try exception in bad_stack(esr: 0x%lx, far: 0x%lx)\n", __func__, esr, far);
...
static void simulate_BAD_STACK_RE(char **argv, int argc)
{
	...
	ret = register_trace_android_rvh_do_sp_pc_abort(secdbg_test_do_sp_pc_abort, NULL);
	if (ret) {
		pr_err("%s: failed to register trace_android_rvh_panic_unhandled(%d)\n", __func__, ret);
		return;
	}

	if (IS_ENABLED(CONFIG_SEC_DEBUG_BAD_STACK_INFO)) {
		secdbg_register_hook_handle_bad_stack(secdbg_test_do_handle_bad_stack);
	} else {
		ret = register_trace_android_rvh_handle_bad_stack(secdbg_test_do_handle_bad_stack, NULL);
		if (ret)
